...
1. Make sure you have the correct file based on you AIQ Deployment location:
...
3. Click on the collection to open it. ![postman15](assets\images\postman15.png)
4. In the **Current Value** fields, set your Entity, Client ID and Secret. Leave the remaining variables unedited: ![postman17](assets\images\postman17.png)
...
2. Search and Open the `TokenGet` Method  and send the request. This will get a token and set it to memory as well as a Refresh token.
3.   Now call any method. The token is automatically added to the call. 
4.   Once the token expires, call `TokenRefresh` to get a new set of tokens. 
...
- **UserKey:** Generated within the AIQ deployment.
- **Entity** to connect get data from. 
1. Make sure you have the correct Postman Collection file based on you AIQ Deployment location:
